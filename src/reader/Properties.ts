...
> {
  ...
  constructor(buffer: Uint8Array) {
    ...
    this._data = AEC.Data.getRootAsData(inBuffer)
  }
  ...
  private getTreeItem(item: AEC.SpatialStructure) {
  ...
  get spatialTree() {
    const item = this._data.spatialStructure(new AEC.SpatialStructure())
    if (!item) return {}
