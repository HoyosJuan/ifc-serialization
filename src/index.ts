...
const run = async (forceSerialize: boolean) => {
  ...
  const attrsFromExpressID = model.getEntityAttributes(expressID)
  ...
  // const start = performance.now()
  // const ids = model.getAllEntitiesOfClass(WEBIFC.IFCWALL).slice(0,1)
  // for (const id of ids) {
  //   const attrs = model.getEntityAttributes(id, {includeRels: true})
  //   console.log(attrs)
  // }
